...
public class HouseOfCardsPokerHand {

	private HouseOfCardsCard myCards[];
	private String pattern;
	private float handValue;
	
	public HandEvaluator(HouseOfCardsCard cards[]) {
		this.myCards = cards;
	}

	public HouseOfCardsCard[] drawRandom(int numberOfRandomCards, HouseOfCardsCard cards[]){
		//Draw two cards
		if (numberOfRandomCards == 2){
			HouseOfCardsCard firstCard = null;
			HouseOfCardsCard secondCard = null;
			int random;
			HouseOfCardsCard tempCard;
			//Create a list of banned card IDs
			int bannedCardIDs[];
			for (HouseOfCardsCard card : cards){
				bannedCardIDs.append(card.getID());
			}

			while (firstCard == null || secondCard == null){
				//Generate a random number
				random = (int) (Math.random() * 52);
				tempCard = new HouseOfCardsCard(random);
				//If it's not in the list of displayed cards, add it
				if (!bannedCardIDs.contains(tempCard.getID())){
					if (firstCard != null){
						secondCard = tempCard;
					}
					else{
		...
		System.out.println("Tried to draw more than two cards!");
		//Failure case
		return null; 
	...
	*
	* @return handValue, which is an integer representing how good a given hand is
	*/

	public int evaluateHand(HouseOfCardsCard cards[]) {
		...
		handValue = tempRank;
		// return pattern in order to get the return val as String
		return handValue;
	}
